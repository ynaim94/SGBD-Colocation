...
create view perso_contrat (ID_CONTRAT_MEMBRE, ID_PERSONNE, ID_COLOCATION, DATE_ENTREE, DATE_SORTIE, NOM_PERSONNE) as
select ID_CONTRAT_MEMBRE, ID_PERSONNE, ID_COLOCATION, DATE_ENTREE, DATE_SORTIE, NOM_PERSONNE
...
drop view compte_total cascade constraints;
create view compte_total (ID_PERSONNE, NOM_PERSONNE, MONTANT, NATURE, CD) as
select PERSONNE.ID_PERSONNE, NOM_PERSONNE, MONTANT_VERSEMENT as MONTANT, 'VERSEMENT' as NATURE, 'CREDIT' as CD
...
union
select ID_PERSONNE, NOM_PERSONNE, MONTANT_ACHAT_PERSONNEL/NOMBRE_DE_PERSONNE, 'ACHAT_PERSONNEL', 'DEBIT'
from (select ID_ACHAT_PERSONNEL as ID_ACHAT, ID_PERSONNE, MONTANT_ACHAT_PERSONNEL, NOM_PERSONNE
     from ACHAT_PERSONNEL left join (perso_contrat inner join BENEFICIAIRE using (ID_CONTRAT_MEMBRE)) using (ID_ACHAT_PERSONNEL)
     group by ID_ACHAT_PERSONNEL, ID_PERSONNE, MONTANT_ACHAT_PERSONNEL, NOM_PERSONNE) inner join nbr_personne_achat using (ID_ACHAT)
where CONCERNE = 'P'
union
select ID_PERSONNE, NOM_PERSONNE, MONTANT_ACHAT_COLOCATION/NOMBRE_DE_PERSONNE, 'ACHAT_COLOCATION', 'DEBIT'
from (select ID_ACHAT_COLOCATION as ID_ACHAT, perso_contrat.ID_PERSONNE, perso_contrat.NOM_PERSONNE, MONTANT_ACHAT_COLOCATION
     from ACHAT_COLOCATION, COLOCATION, perso_contrat
     where ACHAT_COLOCATION.ID_COLOCATION = COLOCATION.ID_COLOCATION
     and COLOCATION.ID_COLOCATION = perso_contrat.ID_COLOCATION
     group by ID_ACHAT_COLOCATION, perso_contrat.ID_PERSONNE, perso_contrat.NOM_PERSONNE, MONTANT_ACHAT_COLOCATION)
     inner join nbr_personne_achat using (ID_ACHAT)
where CONCERNE = 'C';
